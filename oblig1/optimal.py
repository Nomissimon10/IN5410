...
def optimizeSchedule(schedule):
    ...
    prob = pulp.LpProblem("ApplianceScheduling", pulp.LpMinimize)

    # Create variables
    appliance_variables = pulp.LpVariable.dicts("Appliance", ((i, j) for i, a in enumerate(
        schedule) for j in range(a["start"], a["stop"] - a["duration"] + 1)), cat="Binary")

    # Objective function
    prob += pulp.lpSum([appliance_variables[i, j] * estimate_electricity_cost_for_run(j, schedule[i]["duration"], schedule[i]["consumption"]
                       * schedule[i]["duration"] / 1000) for i, a in enumerate(schedule) for j in range(a["start"], a["stop"] - a["duration"] + 1)])

    # Constraints
    for i, a in enumerate(schedule):
        prob += pulp.lpSum([appliance_variables[i, j]
                           for j in range(a["start"], a["stop"] - a["duration"] + 1)]) == 1

    # Solve the linear programming problem
    prob.solve()

    # Update the schedule with the optimized start times
    optimized_schedule = []
    for i, a in enumerate(schedule):
        for j in range(a["start"], a["stop"] - a["duration"] + 1):
            if appliance_variables[i, j].varValue == 1:
    ...
    # Sort schedule by appliance start time
    optimized_schedule = sorted(optimized_schedule, key=lambda x: x["start"])

    peakLoad = calculatePeak(optimized_schedule)[1]
    totalCost = calculatePeak(optimized_schedule)[2]
    print("Outgoing:")
    print("Peak load", peakLoad)
    print("Total cost", totalCost)
